...
export default function AdminEditCategoryPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([catJson, catsJson, usersJson]) => {
        const cat = (catJson as { data?: Record<string, unknown> }).data
        if (cat) {
          setForm({
            name_ru:        String(cat.name_ru        ?? ''),
            slug:           String(cat.slug           ?? ''),
            brief_ru:       String(cat.brief_ru       ?? ''),
            description_ru: String(cat.description_ru ?? ''),
            icon_emoji:     String(cat.icon_emoji     ?? ''),
            wave_color:     String(cat.wave_color     ?? ''),
            wave_accent:    String(cat.wave_accent    ?? ''),
            parent_id:      String(cat.parent_id      ?? ''),
            sub_type:       String(cat.sub_type       ?? ''),
            teacher_id:     String(cat.teacher_id     ?? ''),
            sort_order:     Number(cat.sort_order     ?? 0),
            is_active:      Boolean(cat.is_active     ?? true),
          })
        }
        const catsData = (catsJson as { data?: { id: string; name_ru: string; slug: string; parent_id: string | null }[] }).data
        if (catsData) setTopCategories(catsData.filter(c => !c.parent_id && c.id !== id))
        const usersData = (usersJson as { data?: { users?: Teacher[] } }).data
        if (usersData?.users) setTeachers(usersData.users)
      })
        ...
        <fieldset className="rounded-xl border p-5 space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium">
              Slug *
              {!slugEditable && (
                <button type="button" onClick={() => { setSlugEditable(true); slugTouched.current = true }}
            ...
            <input name="slug" required value={form.slug} onChange={handle}
              ...
              className={`w-full rounded-lg border bg-background px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary/50 ${!slugEditable ? 'cursor-default opacity-70' : ''}`}
              maxLength={80} />
            {slugEditable && (
              <p className="mt-1 text-[11px] text-amber-500">
                Изменение slug сломает старые ссылки. Используйте с осторожностью.
              </p>
            )}
          </div>
